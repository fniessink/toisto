...
Labels are a list of objects that always have three keys: `concept`, `label`, and `language`. The `concept` contains the identifier of the concept that the label describes. The `label` contains the label itself. It is either a string or an object containing different grammatical forms of the label. The `language` contains the language identifier. Currently, the built-in JSON files contain English with identifier `en`, Finnish with identifier `fi`, and Dutch with identifier `nl`.
...
```json
{
    ...
    "labels": [
        ...
        {
            "concept": "she wants ice cream",
            "label": "Hän haluaa jäätelöä.;feminine",
            "language": "fi"
        }
    ]
}
...
```json
{
    ...
    "labels": [
        ...
        {
            "concept": "she wants ice cream",
            "label": "Hän haluaa jäätelöä.;feminine;'jäätelöä is the partitive case of 'jäätelö'",
            "language": "fi"
        }
    ]
}
...
```json
{
    ...
    "labels": [
        {
            "concept": "garlic",
            "label": "valkosipuli;;valko- ('white') + sipuli ('onion')",
            "language": "fi"
        }
    ]
}
...
Add multiple labels if there are multiple way to express the concept in a language, as with "Mikä päivä tänään on?" and "Mikä päivä on tänään?" below. Toisto will quiz the user with each synonym, so in the example below users practicing Finnish will be quizzed about both Finnish sentences.
...
Sometimes a concept in one language can be two different concepts in another language. For example, both in English and Dutch there are separate greetings for the afternoon and the whole day: "Good afternoon" and "Good day" in English and "Goedemiddag" and "Goedendag" in Dutch. In Finnish "Hyvää päivää" is used for both. As an aside, "Hyvää iltapäivää", although grammatically correct, is not used.
    ...
    "labels": [
        {
            ...
            "label": "What do you like?;ice cream",
            "language": "en"
        },
        {
            "concept": "what do you like",
            "label": "Mistä sinä pidät?;jäätelöä",
            "language": "fi"
        },
        {
        ...
        {
            "concept": "i like ice cream.",
            "label": "Minä pidän jäätelöstä.|Pidän jäätelöstä.",
            "language": "fi"
        }
    ]
}
