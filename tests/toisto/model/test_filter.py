...
class FilterTest(ToistoTestCase):
    ...
    def setUp(self) -> None:
        ...
        self.foo = self.create_concept("foo", dict(labels={"en": "foo"}))
        self.bar = self.create_concept("bar", dict(labels={"en": "bar"}))
    ...
    def test_do_not_add_hypernyms_of_selected_concepts(self):
        ...
        little_beer_bar = self.create_concept(
            "little beer bar", dict(hypernym=little_bar.concept_id, labels=dict(en="little beer bar"))
        )
        self.assertEqual(
            {little_beer_bar},
    ...
    def test_do_not_add_holonyms_of_selected_concepts(self):
        ...
        bar_part_part = self.create_concept(
            "bar part part", dict(holonym=bar_part.concept_id, labels=dict(en="bar part part"))
        )
        self.assertEqual(
            {bar_part_part},
            self.filter_concepts(
    ...
    def test_add_antonyms_of_selected_concepts(self):
        ...
        anti_foo = self.create_concept("anti foo", dict(antonym=self.foo.concept_id, labels=dict(en="anti foo")))
        self.assertEqual(
            {self.foo, anti_foo},
    ...
    def test_add_concepts_involved_by_selected_concepts_recursively(self):
        ...
        baz = self.create_concept("baz", dict(involves=zoo.concept_id, labels=dict(en="baz")))
        self.assertEqual(
            {self.bar, baz, zoo},
