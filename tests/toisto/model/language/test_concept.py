...
class ConceptTest(ToistoTestCase):
    ...
    def test_instance_registry(self):
        ...
        concept = self.create_concept(
            "thirty", labels=[dict(label="kolmekymmentä", language=FI), dict(label="dertig", language=NL)]
        )
    ...
    def test_meaning_leaf_concept(self):
        ...
        concept = self.create_concept("one", labels=[dict(label="yksi", language=FI), dict(label="een", language=NL)])
    ...
    def test_meaning_composite_concept(self):
        ...
        concept = self.create_concept(
            "table",
            labels=[
                dict(label=dict(singular="table", plural="tables"), language=EN),
                dict(label=dict(singular="de tafel", plural="de tafels"), language=NL),
            ],
        )
    ...
    def test_meaning_mixed_concept(self):
        ...
        concept = self.create_concept(
            "to eat/third person",
            labels=[
                dict(label="hän syö", language=FI),
                dict(label=dict(feminine="zij eet", masculine="hij eet"), language=NL),
            ],
        )
    ...
    def test_homographs(self):
        ...
        bank = self.create_concept("bank", labels=[dict(label="de bank", language=NL, note="some note")])
        sofa = self.create_concept("sofa", labels=[dict(label="de bank", language=NL)])
    ...
    def test_capitonyms(self):
        ...
        greece = self.create_concept("greece", labels=[dict(label="Kreikki", language=FI)])
        greek = self.create_concept(
            "greek",
            labels=[dict(label="kreikki", language=FI, note="In Finnish, the names of languages are not capitalized")],
        )
        self.assertEqual((greece,), greek.get_capitonyms(Label(FI, "kreikki")))

    def test_is_sentence(self):
        """Test the is-sentence property."""
        self.assertFalse(self.create_concept("sea", labels=[dict(label="sea", language=EN)]).is_complete_sentence)
        self.assertFalse(self.create_concept("greece", labels=[dict(label="Greece", language=EN)]).is_complete_sentence)
        self.assertTrue(self.create_concept("hi", labels=[dict(label="Hei!", language=FI)]).is_complete_sentence)
        self.assertTrue(
            self.create_concept("meaning only", labels=[dict(label="(Hei!)", language=FI)]).is_complete_sentence
        )
        ...
        composite_concept = self.create_concept(
            "the house is big",
            labels=[dict(label={"singular": "The house is big.", "plural": "The houses are big."}, language=EN)],
        )
