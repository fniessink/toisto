...
class ConceptRootsTest(QuizFactoryTestCase):
    ...
    def test_concept_relationship_leaf_concept(self):
        ...
        centre = self.create_concept("centre", dict(labels=dict(fi="keskusta")))
    ...
    def test_concept_relationship_leaf_concept_one_root(self):
        ...
        capital = self.create_concept("capital", dict(roots="city", labels=dict(fi="pääkaupunki", en="capital")))
        city = self.create_concept("city", dict(labels=dict(fi="kaupunki")))
...
class AntonymConceptsTest(QuizFactoryTestCase):
    ...
    def setUp(self) -> None:
        ...
        self.big = self.create_concept("big", dict(antonym="small", labels=dict(en="big")))
        self.small = self.create_concept("small", dict(antonym="big", labels=dict(en="small")))
...
class AnswerConceptsTest(QuizFactoryTestCase):
    ...
    def test_answer_leaf_concepts(self):
        ...
        self.language_pair = EN_NL
        question = self.create_concept("question", dict(answer="answer", labels=dict(en="How are you?")))
        answer = self.create_concept("answer", dict(labels=dict(en="I'm fine, thank you.")))
    ...
    def test_answer_composite_concepts(self):
        ...
        question = self.create_concept(
            "question",
            dict(answer="answer", labels=dict(en=dict(singular="How are you?;singular", plural="How are you?;plural"))),
        )
        answer = self.create_concept(
            "answer",
            dict(labels=dict(en=dict(singular="I'm fine, thank you.", plural="We're fine, thank you."))),
        )
        quizzes = create_quizzes(EN_NL, (), question, answer)
    ...
    def test_multiple_answers(self):
        ...
        question = self.create_concept("question", dict(answer=["fine", "good"], labels=dict(en="How are you?")))
        fine = self.create_concept("fine", dict(labels=dict(en="I'm fine, thank you.")))
        good = self.create_concept("good", dict(labels=dict(en="I'm doing good, thank you.")))
