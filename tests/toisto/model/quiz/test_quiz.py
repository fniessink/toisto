...
from ....base import EN_NL, FI_EN, FI_NL, NL_EN, NL_FI, ToistoTestCase
...
class QuizTest(QuizTestCase):
    ...
    def test_note(self):
        ...
        for question in questions:
            quiz = self.create_quiz(self.concept, question, [Label(NL, "Jij bent|Je bent")])
            hint = " (singular)" if question.question_note == "singular" else ""
...
class OrderQuizTest(QuizTestCase):
    ...
    def test_order_answer(self):
        ...
        quiz = self.create_quiz(self.concept, Label(EN, labels[0]), [Label(EN, "|".join(labels))], ORDER)
        for label in labels:
...
class QuizInstructionTest(QuizTestCase):
    ...
    def test_homographs_get_an_automatic_note_based_on_the_hypernym(self):
        ...
        self.create_concept(
            "furniture",
            labels=[dict(label="het meubilair", language=NL, note="this should not be shown as part of the note")],
        )
    ...
    def test_homographs_get_an_automatic_note_based_on_the_holonym(self):
        ...
        self.create_concept(
            "tree",
            labels=[
                dict(label="puu", language=FI, note="this should not be shown as part of the note"),
    ...
    def test_homographs_get_an_automatic_note_based_on_the_involved_concept(self):
        ...
        self.create_concept(
            "sport", labels=[dict(label="sport", language=EN, note="this should not be shown as part of the note")]
        )
