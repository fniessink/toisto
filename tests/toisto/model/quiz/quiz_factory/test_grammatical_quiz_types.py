...
class GrammaticalQuizTypesTest(QuizFactoryTestCase):
    ...
    @staticmethod
    def grammatical_quiz_action(label1: Label, label2: Label) -> QuizAction | None:
        """Return the quiz action."""
        return GrammaticalQuizType.grammatical_quiz_action(label1, label2)
    ...
    def test_adjective_with_degrees_of_comparison(self):
        ...
        for label in (positive, comparative):
            self.assertEqual(SUPERLATIVE_DEGREE.action, self.grammatical_quiz_action(label, superlative))
        for label in (positive, superlative):
            self.assertEqual(COMPARATIVE_DEGREE.action, self.grammatical_quiz_action(label, comparative))
        for label in (comparative, superlative):
            self.assertEqual(POSITIVE_DEGREE.action, self.grammatical_quiz_action(label, positive))
    ...
    def test_noun_with_grammatical_number(self):
        ...
        self.assertEqual(PLURAL.action, self.grammatical_quiz_action(singular, plural))
        self.assertEqual(SINGULAR.action, self.grammatical_quiz_action(plural, singular))
    ...
    def test_noun_with_grammatical_gender(self):
        ...
        self.assertEqual(MASCULINE.action, self.grammatical_quiz_action(feminine, masculine))
        self.assertEqual(FEMININE.action, self.grammatical_quiz_action(masculine, feminine))
    ...
    def test_noun_with_grammatical_gender_including_neuter(self):
        ...
        for concept in (feminine, neuter):
            self.assertEqual(MASCULINE.action, self.grammatical_quiz_action(concept, masculine))
        for concept in (feminine, masculine):
            self.assertEqual(NEUTER.action, self.grammatical_quiz_action(concept, neuter))
        for concept in (masculine, neuter):
            self.assertEqual(FEMININE.action, self.grammatical_quiz_action(concept, feminine))
    ...
    def test_verb_with_person(self):
        ...
        for concept in (first_person, second_person):
            self.assertEqual(THIRD_PERSON.action, self.grammatical_quiz_action(concept, third_person))
        for concept in (first_person, third_person):
            self.assertEqual(SECOND_PERSON.action, self.grammatical_quiz_action(concept, second_person))
        for concept in (second_person, third_person):
            self.assertEqual(FIRST_PERSON.action, self.grammatical_quiz_action(concept, first_person))
    ...
    def test_verb_with_tense_and_person(self):
        ...
        for present, past in ((present_singular, past_singular), (present_plural, past_plural)):
            self.assertEqual(PAST_TENSE.action, self.grammatical_quiz_action(present, past))
            self.assertEqual(PRESENT_TENSE.action, self.grammatical_quiz_action(past, present))
    ...
    def test_verb_with_infinitive_and_person(self):
        ...
        for concept in (infinitive, singular):
            self.assertEqual(PLURAL.action, self.grammatical_quiz_action(concept, plural))
        for concept in (infinitive, plural):
            self.assertEqual(SINGULAR.action, self.grammatical_quiz_action(concept, singular))
        for concept in (singular, plural):
            self.assertEqual(INFINITIVE.action, self.grammatical_quiz_action(concept, infinitive))
    ...
    def test_verb_with_person_and_number(self):
        ...
        for first_person, second_person in ((nl.ik_heb, nl.jij_hebt), (nl.wij_hebben, nl.jullie_hebben)):
            self.assertEqual(SECOND_PERSON.action, self.grammatical_quiz_action(first_person, second_person))
            self.assertEqual(FIRST_PERSON.action, self.grammatical_quiz_action(second_person, first_person))
        for first_person, third_person in ((nl.ik_heb, nl.zij_heeft), (nl.wij_hebben, nl.zij_hebben)):
            self.assertEqual(THIRD_PERSON.action, self.grammatical_quiz_action(first_person, third_person))
            self.assertEqual(FIRST_PERSON.action, self.grammatical_quiz_action(third_person, first_person))
        for second_person, third_person in ((nl.jij_hebt, nl.zij_heeft), (nl.jullie_hebben, nl.zij_hebben)):
            self.assertEqual(THIRD_PERSON.action, self.grammatical_quiz_action(second_person, third_person))
            self.assertEqual(SECOND_PERSON.action, self.grammatical_quiz_action(third_person, second_person))
    ...
    def test_verb_with_infinitive_and_person_and_number(self):
        ...
        for singular, plural in (
            ...
            self.assertEqual(PLURAL.action, self.grammatical_quiz_action(singular, plural))
            self.assertEqual(SINGULAR.action, self.grammatical_quiz_action(plural, singular))
            self.assertIsNone(self.grammatical_quiz_action(nl.zijn, singular))
            self.assertIsNone(self.grammatical_quiz_action(nl.zijn, plural))
