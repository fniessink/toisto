...
class PracticeTest(ToistoTestCase):
    ...
    def setUp(self) -> None:
        ...
        self.concept = self.create_concept(
            "hi", labels=[dict(label="Terve!", language=FI), dict(label="Hoi!", language=NL)]
        )
    ...
    def test_colloquial_answer_when_question_colloquial(self):
        ...
        concept = self.create_concept(
            "ice cream",
            labels=[
                dict(label="jäätelö", language=FI),
                dict(label="jätski*", language=FI),
                dict(label="het ijsje", language=NL),
            ],
        )
    ...
    def test_answer_with_question_grammar_quiz(self):
        ...
        concept = self.create_concept("house", labels=[dict(label=dict(singular="talo", plural="talot"), language=FI)])
    ...
    def test_quiz_with_multiple_meanings(self):
        ...
        concept = self.create_concept(
            "house",
            labels=[
                dict(label=dict(singular="talo", plural="talot"), language=FI),
                dict(label=dict(singular="huis", plural="huizen"), language=NL),
            ],
        )
        quizzes = create_quizzes(self.language_pair, (PLURAL,), concept)
        expected_feedback = (
    ...
    def test_quiz_third_person_read_quiz(self):
        ...
        concept = self.create_concept(
            "to ride",
            labels=[
                dict(label={"third person": "hän ajaa"}, language=FI),
                dict(label={"third person": dict(feminine="zij rijdt", masculine="hij rijdt")}, language=NL),
            ],
        )
    ...
    def test_quiz_with_example(self):
        ...
        concept = self.create_concept(
            "next to",
            dict(example="the museum is next to the church"),
            labels=[dict(label="vieressä", language=FI), dict(label="naast", language=NL)],
        )
        self.create_concept(
            "the museum is next to the church",
            labels=[
                dict(label="Museo on kirkon vieressä.", language=FI),
                dict(label="Het museum is naast de kerk.", language=NL),
            ],
        )
    ...
    def test_quiz_with_multiple_examples(self):
        ...
        concept = self.create_concept(
            "black", dict(example=examples), labels=[dict(label="musta", language=FI), dict(label="zwart", language=NL)]
        )
        self.create_concept(
            "the car is black",
            labels=[dict(label="Auto on musta.", language=FI), dict(label="De auto is zwart.", language=NL)],
        )
        self.create_concept(
            "the cars are black",
            labels=[dict(label="Autot ovat mustia.", language=FI), dict(label="De auto's zijn zwart.", language=NL)],
        )
        quizzes = create_quizzes(self.language_pair, (DICTATE,), concept)
        expected_feedback = (
    ...
    def test_quiz_with_example_with_synonyms_in_the_target_language(self):
        ...
        concept = self.create_concept(
            "table lamp",
            dict(example="I am looking for a table lamp"),
            labels=[
                dict(label="pöytälamppu", language=FI),
                dict(label="pöytävalaisin", language=FI),
                dict(label="de tafellamp", language=NL),
            ],
        )
        self.create_concept(
            "I am looking for a table lamp",
            labels=[
                dict(label="Minä etsin pöytälamppua.", language=FI),
                dict(label="Minä etsin pöytävalaisinta.", language=FI),
                dict(label="Ik zoek een tafellamp.", language=NL),
            ],
        )
        quizzes = create_quizzes(self.language_pair, (DICTATE,), concept)
