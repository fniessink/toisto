...
class FeedbackTestCase(ToistoTestCase):
    ...
    def test_correct_first_time(self):
        ...
        concept = self.create_concept("hi", dict(labels=dict(nl="hoi", fi="terve")))
    ...
    def test_show_colloquial_language(self):
        ...
        concept = self.create_concept("thanks", dict(labels=dict(nl="dank", fi=["kiitos", "kiitti*"])))
    ...
    def test_show_alternative_answer(self):
        ...
        concept = self.create_concept("hi", dict(labels=dict(nl="hoi", fi=["terve", "hei"])))
        quiz = create_quizzes(NL_FI, (READ,), concept).pop()
    ...
    def test_show_alternative_answers(self):
        """Test that alternative answers are shown."""
        concept = self.create_concept("hi", dict(labels=dict(nl="hoi", fi=["terve", "hei", "hei hei"])))
        quiz = create_quizzes(NL_FI, (READ,), concept).pop()
    ...
    def test_do_not_show_generated_alternative_answers_on_incorrect_guess(self):
        ...
        concept = self.create_concept("house", dict(labels=dict(nl="het huis", fi=["talo"])))
    ...
    def test_instruction_multiple_quiz_types(self):
        ...
        concept = self.create_concept(
            "eat",
            dict(labels=dict(nl={"first person": "ik eet", "third person": dict(feminine="zij eet")})),
        )
    ...
    def test_post_quiz_note(self):
        ...
        concept = self.create_concept("hi", dict(labels=dict(nl="hoi;;'Hoi' is an informal greeting")))
    ...
    def test_multiple_post_quiz_notes(self):
        ...
        concept = self.create_concept(
            "hi", dict(labels=dict(fi="moi;;Moi is an informal greeting;'Moi moi' means goodbye"))
        )
        quiz = create_quizzes(FI_NL, (DICTATE,), concept).pop()
        feedback = Feedback(quiz, FI_NL)
        self.assertIn(
    ...
    def test_post_quiz_note_on_incorrect_answer(self):
        """Test that the post quiz note is formatted correctly."""
        concept = self.create_concept("hi", dict(labels=dict(fi="moi;;'Moi' is an informal greeting")))
        quiz = create_quizzes(FI_NL, (DICTATE,), concept).pop()
        feedback = Feedback(quiz, FI_NL)
        self.assertEqual(
    ...
    def test_post_quiz_example_with_spelling_alternatives(self):
        ...
        hi = self.create_concept("hi", dict(labels=dict(nl="hoi", fi="terve"), example="hi alice"))
        self.create_concept("hi alice", dict(labels=dict(fi="Moi Alice!|Hei Alice!", nl="Hoi Alice!")))
    ...
    def test_post_quiz_example_with_write_quiz(self):
        ...
        hi = self.create_concept("hi", dict(labels=dict(nl="hoi", fi="terve"), example="hi alice"))
        self.create_concept("hi alice", dict(labels=dict(fi="Terve Alice!", nl="Hoi Alice!")))
    ...
    def test_post_quiz_example_with_multiple_meanings(self):
        ...
        hi = self.create_concept("hi", dict(labels=dict(nl="hoi", fi="terve"), example="hi alice"))
        self.create_concept("hi alice", dict(labels=dict(fi="Terve Alice!", nl=["Hoi Alice!", "Hallo Alice!"])))
        quiz = create_quizzes(FI_NL, (WRITE,), hi).pop()
        feedback = Feedback(quiz, FI_NL)
        self.assertEqual(
            Feedback.CORRECT
    ...
    def test_post_quiz_example_with_colloquial_labels(self):
        ...
        hi = self.create_concept("hi", dict(labels=dict(nl="hoi", fi="terve"), example="hi alice"))
        self.create_concept(
            "hi alice", dict(labels=dict(fi=["Terve Alice!", "Moi Alice!*"], nl=["Hallo Alice!", "Hoi Alice!*"]))
        )
        quiz = create_quizzes(FI_NL, (WRITE,), hi).pop()
        feedback = Feedback(quiz, FI_NL)
        self.assertEqual(
    ...
    def test_post_quiz_example_with_synonyms(self):
        ...
        near = self.create_concept(
            "near", dict(labels=dict(fi="lähellä", nl=["dichtbij", "in de buurt"]), example="is is near")
        )
        self.create_concept(
            "is is near", dict(labels=dict(fi="Se on lähellä.", nl=["Het is dichtbij.", "Het is in de buurt."]))
        )
