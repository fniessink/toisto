...
class FeedbackTestCase(ToistoTestCase):
    ...
    def test_correct_first_time(self):
        ...
        feedback_text = feedback_correct(self.guess, self.quiz, self.progress.get_retention(self.quiz))
        self.assertEqual("✅ Correct.\n", feedback_text)

    def test_correct_silenced_for_seconds(self):
        """Test that the correct feedback is given when the user guesses correctly."""
        self.progress.update(self.quiz, True)
        self.progress.get_retention(self.quiz).start = datetime.now() - timedelta(seconds=10)
        self.progress.update(self.quiz, True)
        feedback_text = feedback_correct(self.guess, self.quiz, self.progress.get_retention(self.quiz))
    ...
    def test_correct_silenced_for_minutes(self):
        """Test that the correct feedback is given when the user guesses correctly."""
        self.progress.update(self.quiz, True)
        self.progress.get_retention(self.quiz).start = datetime.now() - timedelta(minutes=10)
        self.progress.update(self.quiz, True)
        feedback_text = feedback_correct(self.guess, self.quiz, self.progress.get_retention(self.quiz))
    ...
    def test_correct_silenced_for_hours(self):
        """Test that the correct feedback is given when the user guesses correctly."""
        self.progress.update(self.quiz, True)
        self.progress.get_retention(self.quiz).start = datetime.now() - timedelta(hours=2)
        self.progress.update(self.quiz, True)
        feedback_text = feedback_correct(self.guess, self.quiz, self.progress.get_retention(self.quiz))
    ...
    def test_correct_silenced_for_days(self):
        """Test that the correct feedback is given when the user guesses correctly."""
        self.progress.update(self.quiz, True)
        self.progress.get_retention(self.quiz).start = datetime.now() - timedelta(days=2)
        self.progress.update(self.quiz, True)
        feedback_text = feedback_correct(self.guess, self.quiz, self.progress.get_retention(self.quiz))
