...
def homonym_tips(concept: Concept, question: Label, *homonym_labels: Label) -> Sequence[str]:
    ...
    Relations are checked from most specific to most general so that a concept with multiple relations
    is disambiguated by its most informative one. For example, when sibling concepts share a hypernym,
    a per-concept 'involves' relation will disambiguate where the shared hypernym cannot.
    """
    ...
    if involved_concepts := concept.get_related_concepts("involves"):
        return [f"involves '{involved.labels(language)[0]}'" for involved in involved_concepts]
