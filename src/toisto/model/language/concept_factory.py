...
from .label import Label, LabelFactory, Labels, label_factory, meaning_factory
...
class LabelJSON(TypedDict):
    """Label as contained in the concept files."""

    concept: ConceptIdListOrString
    label: str | JSONGrammar | JSONRecursiveGrammar
    language: Language
...
class ConceptFactory:
    ...
    def _labels(self, factory: LabelFactory) -> Labels:
        ...
        for label in self.labels:
            if not isinstance(label["label"], dict):
                labels.extend(factory(label["language"], label["label"]))
    ...
    def _constituent_concept_labels(self, category: GrammaticalCategory) -> list[LabelJSON]:
        ...
        for label in self.labels:
            if isinstance(label["label"], dict) and category in label["label"]:
                sliced = LabelJSON(concept=label["concept"], language=label["language"], label=label["label"][category])
