...
class ConceptFactory:
    ...
    def _labels(self) -> Labels:
        ...
        for label in self.labels:
            if isinstance(label["label"], str) and not label["label"].startswith("("):
                note = label.get("note", [])
    ...
    def _meanings(self) -> Labels:
        ...
        return Labels(
            [
                Label(label["language"], label["label"].removeprefix("(").removesuffix(")"))
                for label in self.labels
                if isinstance(label["label"], str) and not label["label"].endswith(Label.COLLOQUIAL_POSTFIX)
            ]
        )
