...
from toisto.metadata import ENCODING, LANGUAGES_FILE
...
def load_languages() -> dict[Language, str]:
    ...
    with Path(LANGUAGES_FILE).open(encoding=ENCODING) as language_registry:
        for line in language_registry:
            if line.startswith("%%"):
                if "Type: language\n" in record_lines:
                    languages.update(_parse_record(record_lines))
                record_lines = []  # Reset the lines for the next record
            elif line.startswith("  "):  # Continuation line
                record_lines[-1] = record_lines[-1].strip() + line[1:]  # Skip one space of the continuation
            else:
                record_lines.append(line)
    return languages


def _parse_record(lines: list[str]) -> dict[Language, str]:
    """Parse a language record."""
    language = _parse_attribute(lines, "Subtag")
    description = _parse_attribute(lines, "Description")
    return {Language(language): description}


def _parse_attribute(lines: list[str], attribute: str) -> str:
